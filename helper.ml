...
let integer_input element =
  let open Html in
  object(self)
    method on_change f =
     Event.add_event_listener element Event.change (fun _ -> f (self # get))

    method element = element

    method get =
      try
        Some (int_of_string (Input.value element))
      with _ -> None

    method set x = Input.set_value element (string_of_int x)
  end
